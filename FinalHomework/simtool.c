...
pernode *per_w;
void tal_insert(char*s,pernode* per_w){//article插入
    int i=0,p=0;
    while(s[i]){
        int c=s[i]-'a';
        if(tal_tree[p][c]==0)
...
int main(int argc,char **argv){
    ...
    arti[ptr].b=b;
    char s[200];
    per_w=(pernode*)malloc(sizeof(pernode));
    pernode *point=per_w;
    point->next=NULL;
    memset(point->count,0,200000);
    while((c=fgetc(article_file))!=EOF){
        ...
        arti[ptr].a=a;
        arti[ptr].b=b;
        point=point->next=(pernode*)malloc(sizeof(pernode));
        point->next=NULL;
        memset(point->count,0,200000);
    }
    end=clock();
    ...
    for(int i=0;i<ptr;i++){
        ...
        for(int j=0;j<n;j++){
            artweight[j]=per_w->count[tal_find(heap->array[j]->word)];
            // printf("%d\n",artweight[j]);
        }
        int sign[1000]={0};
