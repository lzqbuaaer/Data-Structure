...
//统计词频所需字典树以及堆排序函数
#define MAX_WORDS 1000000 // 最大单词数量
...
wTrieNode* root;//总字典树
wTrieNode* dic[100000];//article每篇字典树
char name[100000][30];//每篇名字
int ptr=0;//字典树数量
int weight[100000];//article中每篇权重向量
char fingerprint[100000][1000];//article中每篇指纹
char hash[100000][1000];//hash表
char samname[30];
    ...
    while((c=fgetc(article_file))!=EOF){
        ...
        while(c!='\f'){//一篇文章结束进行一次计算
            if(isalpha(c)){
                ...
                s[p]='\0';//让s结尾
                if(t_find(s)==0){//stopwords中没有这个单词
                    winsert(&root,s);
                    winsert(&dic[ptr],s);
                }
            }
